...
const server = createServer(port, async (req: http.IncomingMessage, res: http.ServerResponse) => {
  try{
    ...
    res.end(JSON.stringify({code: res.statusCode, message: 'Internal Server Error. The server has encountered a situation it does not know how to handle'}));
  } finally{
    res.destroy();
  }
})
